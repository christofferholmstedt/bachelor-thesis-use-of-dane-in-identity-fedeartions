...
The communication standard described in SAML could be more restricted. 
...
Furthermore, the messages that is sent between entities depend on the infrastructure and that data integrity is held.  
The most difficult part about setting up the infrastructure is the first exchanging of keys to set up the first trust 
...
However, to come to this is not that easy, one solution might be a third party, a CA or the use of DNSSEC or maybe both to 
validates that the certificates is the right ones. 
...
information to the certificate sender, that calls the receiver to make sure the message came from him or her by confirming the key used 
in the PGP message. 
...
\subsection{The connecting problem - (Working title)}
\subsubsection{TEST}

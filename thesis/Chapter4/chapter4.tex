...
For example the SAML bindings document says  
...
By changing the SAML documents guidelines to be less open for interpretation it could improve the security. 
...
They would also need sign their own metadata which is not the case in current standards. 
...
Instead of publishing SAML2 certificates in the DNS system it might be possible to just use TLS with TLSA RRs and deem that all the information communicated between any two entities is valid as long as it's over TLS that has been established with TLSA RRs procedures.

...
% TODO: Kolla upp exakt vad OpenSAML är skrivit i för språk och hur det används i Shibboleth.
Shibboleth is not a single program, it's more of a software package consisting of an identity provider written in java and a service provider written in C++ as the main parts in the package.
...
DNS-Based Authentication of Named Entities (DANE) is still a draft for TLS and S/MIME communication and is only available as an informational RFC focusing mainly on TLS use cases.
The main concept is that with DNSSEC in place it's possible to use the DNS system as a trusted source for retrieval of some extra information that confirms what earlier was just a guess work.
As an example it's possible to trust a TLS certificate retrieved when establishing that connection cause it's possible to retrieve extra information through the DNS system that proves it's the right certificate in use.
